...
class DecisionService(DecisionEvaluatorPort):
    def decide(self, check_runs: tuple[CheckRunRecord, ...]) -> DecisionOutcomeCode:
        if any(run.check_business_result_code == CheckBusinessResultCode.FAIL for run in check_runs):
            return DecisionOutcomeCode.REJECTED

        if any(run.check_business_result_code == CheckBusinessResultCode.MANUAL_REVIEW for run in check_runs):
